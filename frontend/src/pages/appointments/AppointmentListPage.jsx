...
export default function AppointmentListPage() {
  ...
  const displayedAppointments = useMemo(() => {
    ...
    return orderedAppointments.filter(
      (appointment) => appointment.status === appointmentCategory,
    );
  }, [orderedAppointments, appointmentCategory, user?.role]);
          ...
          {appointmentCategories.map((category) => {
            ...
            return (
              <Button
                ...
                size="sm"
                variant={
                  appointmentCategory === category ? "default" : "outline"
                }
                onClick={() => setAppointmentCategory(category)}
              >
                {category} ({count})
              </Button>
            );
            ...
            orderedAppointments.length > 0 && (
              <Card>
                <CardContent className="py-10 text-center text-slate-500">
                  No {appointmentCategory.toLowerCase()} appointments.
                </CardContent>
              </Card>
            )}
        </div>
