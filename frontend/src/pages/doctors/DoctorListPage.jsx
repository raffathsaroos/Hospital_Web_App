...
export default function DoctorListPage() {
  ...
  useEffect(() => {
    getDoctorsForAdmin().then((result) => {
  ...
  async function toggleStatus(doctor) {
    ...
    await saveStatus(doctor, nextStatus);
  }

  async function saveStatus(doctor, nextStatus) {
    const result = await setDoctorStatus(doctor._id, nextStatus);
    if (result.error) return toast.error(result.error);
    setDoctors((current) =>
      current.map((item) =>
        item._id === doctor._id ? result.data.doctor : item,
      ),
    );
  ...
  async function confirmDeactivation() {
    ...
    await saveStatus(doctorToDeactivate, false);
      ...
      {!loading && !error && doctors.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center py-12 text-slate-500">
            <Stethoscope className="mb-3 text-blue-600" />
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-1">
                      ...
                      <Button
                        ...
                        size="icon"
                        title={
                          doctor.userId.isActive ? "Mark inactive" : "Activate"
                        }
