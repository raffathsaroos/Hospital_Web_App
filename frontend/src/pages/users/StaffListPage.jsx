...
export default function StaffListPage({ role, slug }) {
  ...
  async function toggleStatus(user) {
    ...
    await saveStatus(user, next);
  }

  async function saveStatus(user, next) {
    const result = await setUserStatus(user.id, next);
    if (result.error) return toast.error(result.error);
    setUsers((current) =>
      current.map((item) => (item.id === user.id ? result.data.user : item)),
    );
  ...
  async function confirmDeactivation() {
    ...
    await saveStatus(userToDeactivate, false);
  ...
  return (
    <div>
      ...
      {!loading && !error && users.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center py-12 text-slate-500">
            ...
            No {role.toLowerCase()} accounts have been added.
          </CardContent>
        </Card>
      )}
      {!loading && !error && users.length > 0 && (
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" asChild>
                        <Link to={`/staff/${slug}/${user.id}/edit`}>
                      ...
                      >
                        {user.isActive ? (
                          ...
                          <ToggleLeft className="text-orange-600" />
                        )}
                      </Button>
                    </div>
                  </TableCell>
      ...
      <ConfirmActionDialog
        ...
        title={`Are you sure you want to mark this ${role.toLowerCase()} inactive?`}
