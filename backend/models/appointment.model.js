...
export const APPOINTMENT_TYPES = ["OPD", "IPD", "Emergency"];
...
const AppointmentSchema = new mongoose.Schema(
  {
    ...
    timeSlot: {
      ...
      trim: true,
      match: [timeSlotPattern, "Time slot must use HH:mm - HH:mm format"],
      validate: {
        validator(value) {
...
AppointmentSchema.pre("validate", function validatePatientSource() {
  ...
  if (hasRegisteredPatient === hasGuestPatient) {
    this.invalidate("patientId", "Provide either patientId or guestPatient");
  }
});

// A terminal status releases the unique slot for another booking.
AppointmentSchema.pre("save", function syncSlotReservation() {
  this.isSlotReserved = ["Pending", "Confirmed", "Paid"].includes(this.status);
});
