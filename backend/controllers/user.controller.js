...
const sendError = (res, error) => {
  ...
  if (error.code === 11000)
    return res
      .status(409)
      .json({ message: "Email, phone, or NIC is already in use." });
  return res
...
const getMe = handle(async (req, res) =>
  res.json({ user: await userService.getUser(req.user._id) }),
);
...
const setStatus = handle(async (req, res) =>
  res.json({
    message: "User status updated successfully.",
    user: await userService.setUserStatus(
      req.params.id,
      req.body.isActive,
      req.user._id,
    ),
