...
const loginUser = async ({ email, password }) => {
  ...
  if (!user.isActive)
    throw createError("Your account is inactive. Please contact admin.", 403);
...
const createUser = async (data) => {
  ...
  if (duplicate)
    throw createError("Email, phone, or NIC is already in use.", 409);
...
const setUserStatus = async (id, isActive, actorId) => {
  ...
  if (id === actorId.toString() && !isActive)
